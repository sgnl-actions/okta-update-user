...
describe('Okta Update User Script', () => {
  const mockContext = {
    env: {
      ENVIRONMENT: 'test'
    },
  ...
  describe('invoke handler', () => {
    ...
    test('should handle API error with errorSummary', async () => {
      ...
      global.fetch = () => Promise.resolve({
        ...
        json: async () => ({
          errorCode: 'E0000007',
          errorSummary: 'Not found: Resource not found: jane.smith@example.com (User)',
          errorLink: 'E0000007',
          errorId: 'oae456'
        })
      });
      ...
      expect(error.message).toBe('Failed to update user: Not found: Resource not found: jane.smith@example.com (User)');
    ...
    test('should properly encode login with special characters', async () => {
      let capturedUrl;
      global.fetch = (url) => {
        capturedUrl = url;
      ...
      // Verify that the URL contains encoded login
      expect(capturedUrl).toContain('test%2Bspecial%40example.com');
    });
  });
  ...
  describe('halt handler', () => {
    test('should handle graceful shutdown', async () => {
      ...
      expect(result.login).toBe('jane.smith@example.com');
