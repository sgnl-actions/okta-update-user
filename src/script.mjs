...
export default {
  /**
   * Main execution handler - updates an existing user in Okta
   * @param {Object} params - Job input parameters
   * @param {string} params.login - User's login/email identifier to update
  ...
  invoke: async (params, context) => {
    ...
    if (response.ok) {
      ...
      console.log(`Successfully updated user ${userData.id} (${login})`);
  ...
  error: async (params, _context) => {
    const { error, login } = params;
    console.error(`User update failed for ${login}: ${error.message}`);
  ...
  halt: async (params, _context) => {
    const { reason, login } = params;
    console.log(`User update job is being halted (${reason}) for ${login}`);
